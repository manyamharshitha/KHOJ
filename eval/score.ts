/**
 * Field-level extraction accuracy against hand-labelled goldens.
 *
 * Runs the real extractor over stored transcripts — no calls are placed. Needs
 * ANTHROPIC_API_KEY. Costs roughly one cent per transcript.
 *
 *   npm run eval
 *
 * Three outcome columns, not one, because the failures are not equivalent:
 *   exact — matched the label
 *   miss  — returned null where the broker did say it   (costs a phone call)
 *   wrong — returned a different value                  (costs a Saturday)
 *
 * `hallucinated` counts fields the model claimed whose quote was not in the
 * transcript. The guard already discarded them; this is how you watch it work.
 */
import { readFileSync } from 'node:fs';
import { join } from 'node:path';
import { config } from '../src/config.js';
import { extractFromTranscript } from '../src/core/extract.js';
import type { Brief, ExtractedFields, Turn } from '../src/types.js';

const SCORED_FIELDS = [
  'available', 'baitPivot', 'rentActual', 'depositMonths',
  'brokerageMonths', 'nonVegAllowed', 'tenantProfile',
] as const;
...
async function main() {
  if (!config.anthropicKey && !process.env.ANTHROPIC_AUTH_TOKEN) {
    console.error(
      'No credentials. Set ANTHROPIC_API_KEY in .env, then re-run.\n' +
  ...
  for (const [file, expected] of Object.entries(goldens)) {
    ...
    for (const field of SCORED_FIELDS) {
      ...
      const got = (extraction as unknown as ExtractedFields)[field] ?? null;
  ...
  // The gate from the build plan: don't move on below 90%.
  if (acc < 90) {
