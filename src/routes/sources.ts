...
export async function sourceRoutes(app: FastifyInstance) {
  /**
   * Read listing pages the seeker points us at, and turn them into listings.
   * One request, a handful of URLs she chose. Not a crawler.
   */
  app.post<{ Body: { urls?: string[] } }>('/api/sources/fetch', async (req, reply) => {
    if (config.authRequired && !requireUser(req, reply)) return reply;
    ...
    for (const url of urls) {
      ...
      // The page text can be enormous; the caller only needs the outcome.
      sources.push({
        url: result.url,
    ...
    // Everything that loaded goes through the same parser as a manual paste, so
    // a fetched page and a pasted block produce identically shaped listings.
